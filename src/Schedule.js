...
class Schedule extends Component {
  ...
  UNSAFE_componentWillMount() {
    $.ajax({
      url: 'http://localhost:3000/createTable.php',
      method: 'POST',
      type: 'post',
      data: {
  ...
  fetchData() {
    ...
    fetch('http://localhost:3000//api.php?table=delivery' + this.props.deliveryDate.replace(/\//g, ""))
    ...
    if (prevState.data !== this.state.data) {
      setTimeout(function () {
        ...
        if(delArray[0] !== undefined){
          console.log(delArray[0].props.customer_no);
        }
  ...
  onCloseButton = (e) => {
    ...
    var delDate = (this.props.deliveryDate);
    $.ajax({
      url: 'http://localhost:3000//updateOpen.php',
      method: 'POST',
      type: 'post',
      data: {
        delDatevar: delDate,
  ...
  render() {
    ...
    cookies.set('deliveryTime', this.props.deliveryDate, {path: 'darpet.com'});
