 ...
 onChange = date => {this.setState({ date })
  ...
  render() {
    ...
    if(this.state.clickedValue === 0){
      ...
      return( 
        <React.Fragment>
        <div className="logged">You are logged as: <b>{this.state.cookies.username}</b><a href='http://localhost:3000/'><button className="logout" onClick={this.deleteAllCookies}>Log out</button></a></div>
      <Calendar
